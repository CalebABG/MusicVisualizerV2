...
import com.calebabg.gui.Controls
import com.calebabg.gui.InstructionsWindow
import com.calebabg.gui.MVUILogger
import com.calebabg.helpers.Utils
import com.calebabg.soundcloud.Playlist
import com.calebabg.soundcloud.SoundCloudApiHelper
import com.calebabg.soundcloud.Track
import com.calebabg.visualizers.*
...
class Visualizer : PApplet() {
    ...
    private fun fetchSoundCloudAudioResource(url: String): ArrayList<AudioResource>? {
        var audioSources: ArrayList<AudioResource>? = null

        val soundCloudResource = SoundCloudApiHelper.get(url)

        if (soundCloudResource != null) {
            audioSources = ArrayList()
    ...
    fun addToMusicQueue(audioResources: ArrayList<AudioResource>?) {
    ...
    private fun addToMusicQueue(resourceKey: String, audioResource: AudioResource) {
    ...
    private fun playNextSong() {
        synchronized(musicQueue) {
            val nextSong: AudioResource = musicQueue.dequeue<AudioResource>() ?: return
    ...
    private fun getSelectedSong(): AudioResource {
        val index = selectedSongIndex()
        return musicQueue.getItem(index)["value"] as AudioResource
    }


    private fun playAudioResource(audioResource: AudioResource, closeLastSong: Boolean = false) {
        try {
            val songTitle = audioResource.title()
            val fullAudioResourcePath = SoundCloudApiHelper.getFullAudioResourcePath(audioResource)

            if (fullAudioResourcePath.isNullOrEmpty()) return
